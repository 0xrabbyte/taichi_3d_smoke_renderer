...
class volumetric:
    ...
    @ti.func
    def get_dens(self, p):
        I = sample((ti.Vector([ti.abs(p[0] - self.st[0]), ti.abs(p[1] - self.st[1]), ti.abs(p[2] - self.st[2])])) / self.h)
        return dens[I]
    @ti.func
    def hit(self, r, t_min, t_max):
        root = -1.0
...
def gen_objects():
    ...
    material_smoke = materials(4, ti.Vector([0.01, 0.01, 0.01]), 1.0)
